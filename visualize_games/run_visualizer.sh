#!/bin/bash
# visualize_games/run_visualizer.sh

set -e

STUDENTID="stud376"
PROJ_DIR="/home/${STUDENTID}/rl_project/hockey_project"
IMAGE_PATH="${PROJ_DIR}/my_env_folder"

# --- Security: Read W&B Key ---
if [ -f "${PROJ_DIR}/private/wandb_api_key.txt" ]; then
    export WANDB_API_KEY=$(cat "${PROJ_DIR}/private/wandb_api_key.txt")
    export WANDB_MODE="online"
else
    export WANDB_MODE="offline"
fi

cd "${PROJ_DIR}"

echo "Running visualizer from ${PROJ_DIR}"

singularity exec \
    --bind /home:/home \
    "${IMAGE_PATH}" \
    python3 visualize_games/render_manager.py \
        --config visualize_games/render_config.yaml
