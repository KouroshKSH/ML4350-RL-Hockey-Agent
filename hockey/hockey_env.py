...
class HockeyEnv(gym.Env, EzPickle):
  ...
  def __init__(
    ...
    random_start: introduced by Kourosh, enables our agent to start from random positions
    """
    ...
    # NOTE: Kourosh added this parameter to the environment, not from professor's original code!
  ...
  def reset(self, one_starting=None, mode=None, seed=None, options=None):
    ...
    if self.mode != Mode.NORMAL:
      ...
      self.player2 = self._create_player(
          (4 * W / 5, H / 2 + self.r_uniform(-H / 4, H / 4)),
          blue,
          True
      )
    ...
    if self.mode == Mode.NORMAL or self.mode == Mode.TRAIN_SHOOTING:
      if self.one_starts or self.mode == Mode.TRAIN_SHOOTING:
        self.puck = self._create_puck((W / 2 - self.r_uniform(H / 8, H / 4),
                                       H / 2 + self.r_uniform(-H / 3, H / 3)), (0, 0, 0))
      else:
        self.puck = self._create_puck((W / 2 + self.r_uniform(H / 8, H / 4),
                                       H / 2 + self.r_uniform(-H / 3, H / 3)), (0, 0, 0))
    ...
    # --- Randomization of our player's starting conditions ---
    if getattr(self, "random_start", False):
      # Randomize Player 1 (our agent) position and velocity
      self.player1.position = (W / 5 + self.np_random.uniform(-W / 12, W / 12),
                               CENTER_Y + self.np_random.uniform(-H / 4, H / 4))
      self.player1.linearVelocity = (self.np_random.uniform(-2, 2), self.np_random.uniform(-2, 2))
      
      # Randomize puck in NORMAL mode (training modes already randomize it)
      if self.mode == self.Mode.NORMAL:
        self.puck.position = (CENTER_X + self.np_random.uniform(-W / 4, W / 4),
                              CENTER_Y + self.np_random.uniform(-H / 3, H / 3))
        self.puck.linearVelocity = (self.np_random.uniform(-5, 5), self.np_random.uniform(-5, 5))
